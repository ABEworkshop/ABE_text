...
\noindent \textbf{Encryption}$(\mathcal{M},S,$ PK$)$~The encryption algorithm takes as input a message $\mathcal{M} \in \mathbb{G}_2 $, a set of attributes $S$ and the public parameters PK. To encrypt the message $\mathcal{M}$ under a set of attributes $S$, it chooses a a random value $\upsilon \in \mathbb{Z}_p $. Then The ciphertext is published as CT=$(S,C'=MY^{\upsilon},\{C_{i}=T_{i}^{\upsilon}\}_{i\in S})$;

\noindent \textbf{Key Generation}$(\mathcal{T},$ MK$)$~The Key Generation algorithm takes an access structure $\mathcal{T}$ and the master key MK, then outputs a secret key SK. The SK enables the user to decrypt a message encrypted under a set of attributes $S$ if and only if $\mathcal{T}(S)$ = 1. First let the access structure $\mathcal{T}$ be a tree, then choose a polynomial $q_x$ for each node $x$ (including the leaves) in the tree $\mathcal{T}$.
...
and return the result.\\
 After computing, the equation DecryptNode$($CT$,$ SK$,r) = e(g, g)^{y\upsilon}= Y^\upsilon $ is true if and only if the ciphertext satisfies the tree. Since, $C'=MY^\upsilon$s the decryption algorithm simply divides out $Y^\upsilon$ and recovers the message $\mathcal{M}$.
...
\newline\newline Encrypt(PK, $\mathcal{M}$, A).The encryption algorithm takes as input the public parame- ters PK, a message $\mathcal{M}$, and an access structure A over the universe of attributes. The algorithm will encrypt $\mathcal{M}$ and produce a ciphertext CT such that only a user that possesses a set of attributes that satisfies the access structure will be able to decrypt the message. We will assume that the ciphertext implicitly contains A.\newline\newline
