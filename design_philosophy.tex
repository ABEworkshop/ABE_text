...
Before discuss "attribute" and "access control", we first introduce a new notion, authorization. 
The authorization informally represent the process of obtaining the permission to decrypt by users. 
Based above, we claim that "access control" is that if and only if some objects from user satisfy some specific rule from data owner, the user can be authorized.
When access control occurs to ABE, the least fineness which still has the function of objects on authorization turns into "attribute", instead of "identity" in IBE. 
In addition, in ABE, specific rule mentioned above means access policy or more concrete access structure which will be detailedly  discussed in our next.
Therefore, the prerequisite of authorization can be explained as "attributes satisfy access policy". 


On convention and also for convenience, attributes a participant owns are collected in a set called attribute set. 
Moreover, the attribute set which satisfies specific access policy is called authorized attribute set,$\gamma$, shown on Table ~\ref{tab:notions}. 
However, due to the fact that data owners incline to more efficient, flexible and practical way in access control, 
there are various expressions on access policy in ABE. 
