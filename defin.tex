...
$\lambda \in \mathbb{N}$ and $1^\lambda$ denotes the string of $\lambda$ ones.
...
\item\textbf{Encrypt}$(\mathcal{M},$ PK$,Y)$~This is a randomized algorithm that takes as input a message $\mathcal{M}$, the public parameters PK and a ciphertext index $Y$. It outputs the ciphertext CT$_Y$.
\item\textbf{Decrypt}$($PK$,$ CT$_Y,$ SK$_X)$~The decryption algorithm takes as input the public parameters PK, a ciphertext CT$_Y$, the decryption key SK$_X$. It outputs the result of decrypt $\mathcal{M}$.
\end{itemize}
...
\textbf{Decrypt}(PK$,$ \textbf{KeyGen}(MK$,$ PK$,X$), \textbf{Encrypt}($\mathcal{M},$ PK$,Y$))=$\mathcal{M}$\\
...
This issue of ABE bears some resemblance the one of identity-based encryption \cite{Dan:CRYPTO'01}.
%紧抓变化和限制
...
instead of the same thing at the positions of $X$ and $Y$ for ABE (identity strings ID)\cite{Dan:CRYPTO'01}.
The reason is that
...
\begin{itemize}
  ...
  Then runs SK$_X \leftarrow$ \textbf{KeyGen}$($MK$,$ PK$,X)$ and $\mathcal{M} \leftarrow$ \textbf{Decrypt}$($PK$,$ CT$_Y,$ SK$_X)$.
...
additionally, due to the fact that the practical combinations of oracles in ABE is not so many \cite{Zhiguang:ITSC'16},
...
For the security parameter $k \in \mathbb{N}$ and $atk \in \{CPA, CCA1, CCA2\}$,
the indistinguishability game is defined by the following experiment.
\end{ABEgame}
\textbf{Exp}$_{\Sigma,\mathcal{A},b}^{IND-atk}(k)$:
\begin{eqnarray*}
  (\text{MK}, \text{PK}) &\leftarrow& \textbf{Setup}(\lambda,U); \\
  (M_0,M_1,Y^\ast) &\leftarrow& \mathcal{A}_1^{\mathcal{O}_1}(\text{PK}); \\
  b &\leftarrow& \{0, 1\}; \\
  CT^\ast &\leftarrow& \textbf{Encrypt}(\text{PK},Y^\ast,M_b); \\
  b^\prime &\leftarrow& \mathcal{A}_2^{\mathcal{O}_2}(\text{PK},CT^\ast,M_0,M_1,Y^\ast).
\end{eqnarray*}
...
\begin{ABEsec}
For $\lambda \in N$, the ABE scheme $\Sigma$ is said to be \textbf{IND-ABE-atk} secure
...
Although Vipul Goyal $et~al.$  and John Bethencourt $et~al.$ content that
...
Ling Cheung and Calvin Newport \cite{Ling:CCS'07} apply the CHK technique to construct a IND-ABE-CCA2-secure CP-ABE scheme.
Given above IND-ABE-CCA2-secure ABE scheme involve one-time signatures due to the CHK technique,
Weiran Liu $et~al.$ \cite{Weiran:ESORICS'14} exploit Chameleon hash to achieve IND-ABE-CCA2-secure KP-ABE scheme without one-time signature.
