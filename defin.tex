...
Similarly, the decryption oracle $\mathcal{O}_{dec}$ for ABE have the same restriction.
...
we discuss the two oracles involved in the security games for ABE scheme, 
which can be queried by the adversary in an adaptive manner in the security game.  
In addition, the restriction of non-trivial query mentioned above should be met. 
\begin{itemize}
  ...
  Otherwise, those inputs $X_j$ satisfying $f_{as}(X_j,Y)=1$ are invalid input for $\mathcal{O}_{keygen}$. 
  \item  Odec: On input % X 还是Y? phase1 有没有$f_{as}(X_j,Y)不等于1$ 的限制？
\end{itemize}
