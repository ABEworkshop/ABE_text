...
For the security parameter $k \in N$ and $atk \in \{CPA, CCA1, CCA2\}$,
...
\begin{eqnarray*}
  ...
  (m_0, m_1, Y^\ast) &\leftarrow& \mathcal{A}_1^{\mathcal{O}_1}(PK); \\
  b &\leftarrow& \{0, 1\}; \\
  CT^\ast &\leftarrow& \textbf{Encrypt}(PK, Y^\ast, m_b); \\
  b^\prime &\leftarrow& \mathcal{A}_2^{\mathcal{O}_2}(PK, CT^\ast, m_0, m_1, Y^\ast).
\end{eqnarray*}
