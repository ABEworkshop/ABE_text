...
The efficiency and communication cost comparison is presented in table 1 and table 2 respectively.
\begin{table*}[htbp]
\centering
 \caption{\label{tab:test}Efficiency Comparison }
...
\begin{table*}[htbp]
\centering
 \caption{\label{tab:test}Communication Cost Comparison }
 \begin{tabular}{cccc}
  \toprule
...
In Distributed systems, it will not be user-friendly for access rules for objects to be based on identities considering the various dynamic sets of users in todays computing environment. In light of the above, the incipient proposal of Attribute Based Encryption in a landmark work by A. Sahai and B. Waters in 2005\cite{Sahai:EUROCRYPT'05} and later by Goyal et al\cite{Goyal:CCS'06} opened up various interests to the research community.
...
On the other hand, Wang et al\cite{Jinguang:ESORICS'14} gave a security analysis of PPDCP-ABE scheme of Han et al\cite{Aijun:TPDS'13} and point out the security weakness of their scheme. It came out that their basic decentralized ciphertext-policy ABE scheme cannot resist collusion attacks. Also, the privacy-preserving key extract protocol proposed by\cite{Aijun:TPDS'13}, allows the authority to reveal users' credentials, hence, the privacy protection of attributes cannot be provided.
