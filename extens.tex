...
\subsection{Proxy Re-Encryption (by Yi Wang)}

Proxy Re-Encryption (PRE) is a cryptographic primitive first introduced by Blaze et.al in 1998. In a PRE scheme, a proxy is allowed to transform a ciphertext encrypted by Alice's public key into another one that can be decrypted by Bob's secret key, this process is called re-encryption. Considering this case, an encrypted email is send to Alice, but Alice is off-line. Alice wishes Bob could still read the message in her encrypted email. Therefore, Alice delegates a re-encryption key to the proxy. With the key, the proxy could transform the email to one that could be decrypted by Bob's secret key. With the PRE system, Bob could decrypt a re-encrypted email of the same message with his own secret key.

In PRE system, the proxy is semi-trusted, but the user(Alice) doesn't need to delegate part of her decryption capability to others including the proxy. There is also an advantage that the user could finish any decryption only with his own secret key. So storing any additional decryption key is unnecessary. Otherwise, the proxy authority can only translate one ciphertext into another one under another public key. It is impossible for the delegation to obtain the corresponding plaintext and the original secret keys. With these advantages, PRE can be widely used into many public key cryptosystem. It also have many  practical applications. For example, encrypted email forwarding, distribute file system, and the DRM(Digital Rights Management) of Apple's iTunes.

Since PRE have been introduced in 1998, there are many related works on it. In the paper \cite{Matt:EUROCRYPT'98}, it proposed the first concrete bidirectional PRE scheme. This scheme allows the key holder to publish the proxy function. It applied by untrusted parties without further involvement by the original key holder. Their scheme also has multi-use property. Ateniese \emph{et al.} \cite{Giuseppe:TISSEC'06} presented the first unidirectional. It is a single-use proxy re-encryption scheme. In 2007, Green and Ateniese\cite{Matthew:ACNS'07} provided identity-based PRE. The security of it is in the random oracle model. Chu \emph{et al.} \cite{Chu:ISC'07} proposed new identity-based proxy re-encryption schemes in the standard model. Matsuo \cite{Toshihiko:ICPC'07} also proposed new  identity-based proxy re-encryption system, the solution needs a re-encryption key generator (RKG) to generate re-encryption keys. Guo \emph{et al.} \cite{Shanqing:WUJ'08} proposed the first attribute-based proxy re-encryption(ABPRE) scheme, but their scheme is based on key policy and bidirectional. 
Liang \emph{et al.} \cite{Xiaohui:ISICCS'09} proposed the first ciphertext policy attribute-based proxy re-encryption(CP-ABPRE) scheme which has the above properties except re-encryption control. In TCC 2012, Chandran \emph{et al.} \cite{Shanqing:TTC'12} proposed an obfuscation for functional re-encryption with collusion resistant property. Recently, a CCA secure CP-ABPRE was proposed in \cite{Kaitai:INCS'13}, in this paper the scheme is proven in the random oracle model. In\cite{Kaitai:ISPEC'14}, a CP-ABPRE system was built and proven secure against CCA in the standard model.\\

Next, we will give an ABPRE model in\cite{Xiaohui:ISICCS'09}.\\

Definition:  \emph{An} ABPRE  \emph{scheme is a tuple of probabilistic polynomial time algorithms} (\textbf{Setup}, \textbf{KeyGen}, \textbf{RKGen}, \textbf{Enc}, \textbf{ReEnc}, \textbf{Dec}).

\begin{itemize}
\item \textbf{Setup}$(1^\lambda) \rightarrow (pp,mk)$: On input a security parameter $1^\lambda$, the setup algorithm \textbf{Setup} outputs a system public parameter \emph{pp} and a master key $mk$.
\item \textbf{KeyGen}$(S,mk) \rightarrow (usk)$: On input an index set \emph{$S^1$} and a master key $mk$, the key generation algorithm \textbf{KeyGen} outputs a secret key $usk$.
\item \textbf{RKGen}$(usk,\mathbb{A}) \rightarrow (rk)$: On input a secret key $usk$ and an access structure $\mathbb{A}$, the re-key generation algorithm \textbf{RKGen} outputs a re-key $rk$.
\item \textbf{Enc}$(\mathbb{A},\mathcal{M}) \rightarrow (CT)$: On input an access structure $\mathbb{A}$ and a message $\mathcal{M}$, the encryption algorithm \textbf{Enc} outputs a ciphertext $CT$.
\item \textbf{ReEnc}$(rk,CT) \rightarrow (CT')$: On input a re-key $rk$ and a ciphertext $CT$, the re-encryption algorithm \textbf{ReEnc} first checks if the index set in $rk$ satisfies the access structure of $CT$. Then, if check passes, it outputs a re-encrypted ciphertext $C'$; otherwise, it outputs ``reject''.
\item \textbf{Dec}$(usk,CT) \rightarrow (\mathcal{M})$: On input a secret key $usk$ and a ciphertext $CT$, the decryption algorithm \textbf{Dec} first checks if the index set in $usk$ satisfies the access structure of $CT$. Then, if check passes, it outputs a message $\mathcal{M}$ in the message space; otherwise, it outputs ``reject''.
\end{itemize}

Then, we present the construction with six algorithms \textbf{Setup}, \textbf{KeyGen}, \textbf{ENC}, \textbf{RKGen}, \textbf{ReEnc}, \textbf{Dec}.\\

\noindent
\textbf{Setup}$(1^\lambda)$ Generate a bilinear group $\mathbb{G}$ of prime order $p$, with bilinear map
$e: \mathbb{G}\times\mathbb{G} \rightarrow \mathbb{G}_{T}$. Next, it selects elements $y$, $t_{i}$ ($1\leq i \leq3n$) in $\mathbb{Z}_{p}$ and two generators $g, h$ of $\mathbb{G}$ at random. Let $Y=e(g,h)^y$ and $T_{i}=g^{t_{i}}$, $T'_{i}=h^\frac{1}{t_{i}}$, for each $1\leq i \leq3n$. The public parameter $pp$ includes
$\langle e, g, h, Y, \{T_{i}, T'_{i}\}_{1\leq i\leq 3n} \rangle$.
The master key $mk$ is $\langle y, \{{t_{i}}\}_{1\leq i\leq 3n} \rangle$.\\

\noindent
\textbf{KeyGen}$(S,mk)$ Let $S$ denote an index set of attributes. It chooses random $r_{1}\cdots r_{n}$ from $\mathbb{Z}_{p}$ and sets $r=r_{1}+r_{2}+\cdots+r_{n}$. Compute $\hat{D}=h^{y-r}$,  and for each $i\in\mathcal{N}(\mathcal{N}=\{1,2,\cdots,n\})$, if $i\in S$, $D_{i,1}=h^\frac{r_{i}}{t_{i}}$,$D_{i,2}=h^\frac{r_{i}}{t_{2n+i}}$. Otherwise, $D_{i,1}=h^\frac{r_{i}}{t_{n+i}}$, $D_{i,2}=h^\frac{r_{i}}{t_{2n+i}}$. It outputs a user's secret key $usk=\langle S, {(D_{i, 1}, D_{i, 2})}_{i\in\mathcal{N}},\hat{D} \rangle$.\\

\noindent
\textbf{Enc}$(\mathcal{M},\mathbb{A})$ Let $\mathbb{A}$ denote an access structure. To encrypt a message $\mathcal{M} \in G_{T}$, it selects random $s \in Z_{p}$ and computes $\tilde{C}=\mathcal{M} \cdot Y^s$, $\hat{C}=g^s$, $\breve{C}=h^s$. For $i \in \mathcal{N}$, if $+d_{i}$ appears $\mathbb{A}$, $C_{i}=T^s_{i}$; if $-d_{i}$ appears $\mathbb{A}$, $C_{i}=T^s_{n+i}$; otherwise $C_{i}=T^s_{2n+i}$. It outputs $CT=\langle \mathbb{A}, \tilde{C}, \hat{C}, \breve{C}, (C_{i})_{(i \in \mathcal{N})} \rangle$.\\

\noindent
\textbf{RKGen}$(usk,\mathbb{A})$ Let $usk$ denote a valid secret key consisting of $\langle S,{(D_{i, 1},D_{i, 2})}_{i\in\mathcal{N}}, \hat{D} \rangle$, and $\mathbb{A}$ denote an access structure. It selects random $d\in \mathbb{Z}_{p}$ and set $\mathfrak{D}=g^d, \hat{D}'=\hat{D}$. For $i\in\mathcal{N}$, if $i\in S$, $D_{i, 1}'=D_{i, 1}\cdot(T_{i}')^d$, $D_{i, 2}'D_{i, 2}\cdot(T_{2n+i}')^d$; otherwise, $D_{i, 1}'=D_{i, 1}\cdot(T_{n+i}')^d$, $D_{i, 2}'=D_{i, 2}\cdot(T_{2n+i}')^d$; $\mathfrak{C}$ is the ciphertext of $\mathfrak{D}$ under the access structure $\mathbb{A}$.

It outputs $rk=\langle S,\mathbb{A}, {(D'_{i, 1}, D'_{i, 2})}_{i\in\mathcal{N}},\hat{D}',\mathfrak{C} \rangle$.\\

\noindent
\textbf{ReEnc}$(rk,CT)$ Let $rk$ denote a valid re-key consisting of $\langle S,\mathbb{A}',{(D'_{i, 1}, D'_{i,2})}_{i\in\mathcal{N}}, \hat{D}',\mathfrak{C} \rangle$ and $CT$ denote a well-formed ciphertext $\langle \mathbb{A},\tilde{C},\hat{C},\breve{C},(C_{i})_{(i \in \mathcal{N})} \rangle$, It checks if $S$ satisfies $\mathbb{A}$, if not, output $\bot$; otherwise, for $i\in\mathcal{N}$:
\begin{itemize}
\item $+d_{i}$ appears in $\mathbb{A}$, $E_{i}=e(C_{i}$, $D'_{i, 1})=e(g^{t_{i}s}$, $h^\frac{r_{i}+d}{t_{i}})=e(g,h)^{s(r_{i}+d)}$;
\item $-d_{i}$ appears in $\mathbb{A}$, $E_{i}=e(C_{i}$, $D'_{i, 1})=e(g^{t_{n+i}s}$, $h^\frac{r_{i}+d}{t_{n+i}})=e(g,h)^{s(r_{i}+d)}$;
\item otherwise, $E_{i}=e(C_{i}$, $D'_{i, 1})=e(g^{t_{2n+i}s}$, $h^\frac{r_{i}+d}{t_{2n+i}})=e(g, h)^{s(r_{i}+d)}$;
\end{itemize}

It then computes $\bar{C}=e(\hat{C},\hat{D}')\prod_{i\in\mathcal{N}}E_{i}=e(g^s,h^{y-\sum^n_{i=1}r_{i}})\cdot e(g,h)^{nds+s\sum^n_{i=1}r_{i}}=e(g, h)^{nds+ys}$; output a re-encrypted
ciphertext $CT'=\langle \mathbb{A}',\tilde{C},\bar{C},\breve{C},\mathfrak{C} \rangle$.

\textbf{Dec}$(CT,usk)$ Let $usk$ denote a valid secret key $\langle S,{(D_{i, 1}, D_{i, 2})}_{i\in\mathcal{N}},\hat{D} \rangle$. It checks if $S$ satisfies $\mathbb{A}$, if not, output $\bot$; otherwise, do
\begin{enumerate}
\item If $CT$ is an original well-formed ciphertext consisting of $\langle \mathbb{A},\tilde{C},\hat{C},\breve{C},(C_{i})_{(i\in\mathcal{N})} \rangle$, for $i\in\mathcal{N}$:
    \begin{itemize}
\item $+d_{i}$ appears in $\mathbb{A}$, $E_{i}=e(C_{i}$, $D_{i, 1})=e(T^s_{i}$,$h^\frac{r_{i}}{t_{i}})=e(g, h)^{sr_{i}}$;
\item $-d_{i}$ appears in $\mathbb{A}$, $E_{i}=e(C_{i}$, $D_{i, 1})=e(T^s_{n+i}$,$h^\frac{r_{i}}{t_{n+i}})=e(g, h)^{sr_{i}}$;
\item otherwise, \emph{$E_{i}=e(C_{i}, D_{2n+i, 1})=e(T^s_{i}, h^\frac{r_{i}}{t_{2n+i}})=e(g, h)^{sr_{i}}$};
    \end{itemize}
It outputs $\frac{\tilde{C}}{e(\hat{C},\hat{D})\cdot\prod_{i\in\mathcal{N}}E_{i}}=\frac{m\cdot e(g, h)^{ys}}{e(g,h^{y-r})\cdot e(g, h)^{sr}}=\mathcal{M}$.
\item Else if $CT$ is a re-encrypted well-formed ciphertext consisting of $\langle \mathbb{A}',\tilde{C},\bar{C},\breve{C},\mathfrak{C} \rangle$, it decrypts $\mathfrak{C}$ using $usk$ and obtains $\mathfrak{D}=g^d$. Then, it outputs $\frac{\tilde{C}e(\mathfrak{D},\breve{C})^n}{\bar{C}}=\frac{m\cdot e(g,h)^{ys}\cdot e(g^d, h^s)^n}{e(g,h)^{ys+nds}}=\mathcal{M}$.
\item Else if $CT$ is a multi-time re-encrypted well-formed ciphertext, decryption is similar with the above phases.
\end{enumerate}
...
Even though some prior researchers\cite{Yuan:ICWS'05}\cite{Adelsbach:ICISC'06}\cite{Kapadia:NDSS'07}\cite{Bradshaw:CCS'04} proposed some form of multi-authority, it was noted by M黮ler \emph{et al.} \cite{Sascha:ICISC'08} that the techniques used in these applications are not collusion-resistant, so they can not be classified as ABE.
...
M黮ler \emph{et al.} secured their scheme in the random oracle model and provided a security proof using the adaptive model.
...
\noindent\textbf{System}  It output the system public key and security parameter and system public key $Y_{0}$.   \\
\textbf{Attribute Authority $k$} For authority $k$, authority secret key ($s_{k}$,$t_{k,1}$...$t_{k,n}$), and authority public key ($T_{k,1}$...$T_{k,n}$) is established in this step. Besides, user u would have a unique secret key $D_{k,i}$ here.\\
\textbf{Central Authority}  Every authority gets a central authority secret key $s_{k}$ here. Like other authority, a secret key $D_{CA}$ would be established. Noteworthy is that includes the secret key of all authorities.   \\
\textbf{Encrypt}  this algorithm that takes a message m as input and the cipher text as output($E,E_(CA)$).\\
...
\indent On the other hand, central authority are prone to bring certain risks that if it was corrupted, the whole scheme would be broken. Due to reducing the severe reliance on central authority, the Secure threshold multi authority attribute based encryption without a central authority was introduced\cite{Huang:INDOCRYPT'08}. Two protocols, distributed key generation protocol (DKG) and joint zero secret sharing protocol (JZSS), make the major contribution.Another approach is that several authorities could be in charge of the qualified specific attributes' key distribution in Efficient Statically-Secure Large-Universe Multi-Authority Attribute-Based Encryption\cite{Yannis:FC'15}.
\\
...
%\end{table}
\\
\indent Table~\ref{tab:commult} contains the comparison in the length of user's key and ciphertext and performance comparison. t$_{e1}$,t$_{e2}$ denotes the time spent on two different bilinear group operations, $t_{p}$ denotes the time spent on pairing operation, $k$ means the number of authority. The table compares the efficiency of diverse scheme.
\\
\begin{table*}
...
\begin{equation*}
\begin{array}{ccc}
 C=\mathcal{M}\cdot(e(g,g)^{\alpha})^s, & C^{(1)}=g^s, & C_i^{(2)}=g^{a\lambda_{i}}F(\rho(i))^{-s},\\
 &C_j^{(3)}=g^{b\cdot s_j}, & C_j^{(4)}=(g^{b^2\cdot ID_j}v^b)^{s_j}.\\
\end{array}
\end{equation*}
...
Some comparison of the efficiency of the schemes are shown on Table~\ref{tab:comvoca}.\\
%\begin{table*}[t]
%\centering  % 表居中
...
\hline
\cite{Nuttapong:ICPC'09}(scheme1)&KP&$\surd$&$\times$&$(\omega+2)|G_1|+|G_2|$&$2l_0|G_1|$&$(2l_0+2)P+l_0E_{G2}$\\
\hline
\cite{Nuttapong:ICPC'09}(scheme2)&KP&$\surd$&$\times$&$(2r+\omega+1)|G1|+|G2|$&$(2l_0+2)|G1|$&$(2l_0+2r)P+(l_0+r)E_{G2}$\\
\hline
\cite{Nuttapong:ICPC'09}(scheme1)&CP&$\surd$&$\times$&$(l_s+2)|G1|+|G2|$&$(2l_s+3)P+l_sE_{G2}$&$3|G1|$\\
\hline
\cite{Nuttapong:ICPC'09}(scheme2)&CP&$\surd$&$\times$&$4|G1|+|G2|$&$5|G1|$&$(2l_s+2r+1)P+(l_s+r)E_{G2}$\\
\hline
